...
async function renderProfileUI() {
    ...
    document.getElementById('shareProfileBtn').onclick = async () => {
        const shareData = { title: `${currentUser.name} on MEET`, text: `Check out ${currentUser.name}'s profile on MEET – where sparks glow! 💖`, url: `https://ceezy-website.web.app?ref=${currentUser.referralCode}` };
        if (navigator.share) { try { await navigator.share(shareData); } catch (err) {} } else { await navigator.clipboard.writeText(shareData.url).then(() => customAlert("Profile link copied!", "Share")); }
    };
...
// ========== ALL EVENT BINDINGS (DOMContentLoaded) ==========
function bindAllEvents() {
    ...
    document.getElementById('appealBanBtn')?.addEventListener('click', async () => {
        ...
        if (usersSnap.empty) { customAlert("No account found.", "Not Found"); return; }
        const userDoc = usersSnap.docs[0]; const userData = userDoc.data();
        if (!userData.banned) { customAlert("This account is not banned.", "Not Banned"); return; }
        const reason = await customPrompt("Why should you be unbanned?", "", "Appeal");
        if (reason) { await db.collection("appeals").add({ userId: userDoc.id, reason, status: "pending", timestamp: Date.now() }); customAlert("Appeal submitted.", "Appeal Submitted"); }
    });
    document.getElementById('googleSignInBtn')?.addEventListener('click', async () => { try { const user = await window.signInWithGoogle(); localStorage.setItem('currentUserUid', user.uid); currentUser = (await db.collection("users").doc(user.uid).get()).data(); showMainApp(); } catch(err) { customAlert(err.message, "Error"); } });
    document.getElementById('googleSignUpBtn')?.addEventListener('click', async () => { try { const user = await window.signInWithGoogle(); localStorage.setItem('currentUserUid', user.uid); currentUser = (await db.collection("users").doc(user.uid).get()).data(); showMainApp(); } catch(err) { customAlert(err.message, "Error"); } });
    document.getElementById('signupFormElem')?.addEventListener('submit', async (e) => {
        ...
        if(pwd !== confirm) { customAlert("Passwords mismatch", "Error"); return; }
        const gender = document.getElementById('signupGender').value, age = document.getElementById('signupAge').value, refCode = document.getElementById('signupReferralCode').value;
        try { await window.signupUser(email, pwd, name, age, gender, refCode); document.getElementById('signupView').style.display = 'none'; document.getElementById('loginView').style.display = 'flex'; } catch(err) { customAlert("Signup failed: " + err.message, "Error"); }
    });
    document.getElementById('loginFormElem')?.addEventListener('submit', async (e) => {
        ...
        try { const user = await window.loginUserFirebase(email, pwd); localStorage.setItem('currentUserUid', user.uid); currentUser = (await db.collection("users").doc(user.uid).get()).data(); showMainApp(); } catch(err) { customAlert("Login failed: " + err.message, "Error"); }
    });
    ...
    document.querySelectorAll('.toggle-pwd').forEach(icon => { icon.addEventListener('click', function() { const target = document.getElementById(this.dataset.target); if(target) { target.type = (target.type === 'password') ? 'text' : 'password'; this.classList.toggle('fa-eye'); this.classList.toggle('fa-eye-slash'); } }); });
    ...
    document.getElementById('chatSearchInput')?.addEventListener('input', function(e) {
        ...
        document.querySelectorAll('.chat-list-item').forEach(item => { item.style.display = (item.querySelector('.chat-name')?.textContent?.toLowerCase() || '').includes(term) ? 'flex' : 'none'; });
    });
...
function startApp() {
    if(localStorage.getItem('currentUserUid')) {
        loadCurrentUser().then(() => { if(currentUser) showMainApp(); else document.getElementById('loginView').style.display = 'flex'; });
    } else { document.getElementById('loginView').style.display = 'flex'; }
}
if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', () => { bindAllEvents(); startApp(); }); }
else { bindAllEvents(); startApp(); }
