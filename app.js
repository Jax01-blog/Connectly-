...
async function openChatScreen(partnerId) { /* unchanged */ }
...
// ========== EVENT BINDINGS (ALL WRAPPED IN DOMContentLoaded) ==========
document.addEventListener('DOMContentLoaded', function() {
    // Auth navigation
    ...
    document.getElementById('forgotPasswordBtn')?.addEventListener('click', async () => {
        ...
        if(email) {
            auth.sendPasswordResetEmail(email)
                .then(() => customAlert("Reset email sent! Check your inbox.", "Email Sent"))
                .catch(err => customAlert("Error: " + err.message, "Error"));
        }
    });
    ...
    document.getElementById('appealBanBtn')?.addEventListener('click', async () => {
        ...
        if (reason) {
            await db.collection("appeals").add({ userId: userDoc.id, reason, status: "pending", timestamp: Date.now() });
    ...
    document.getElementById('uploadIntroBtn')?.addEventListener('click', () => {
        ...
        if(file) uploadIntro(file); else customAlert("Select a file first", "Error");
    });
    ...
    document.getElementById('aiChatToggleBtn').addEventListener('click', () => {
        if (!currentUser) { customAlert("Please log in to use MEET AI.", "Authentication Required"); return; }
...
(function() {
    ...
    if (ref) {
        const lg = document.getElementById('loginReferralGroup');
        if(lg) lg.style.display = 'block';
        const li = document.getElementById('loginReferralCode');
        if(li) li.value = ref;
        const si = document.getElementById('signupReferralCode');
        if(si) si.value = ref;
    }
})();
